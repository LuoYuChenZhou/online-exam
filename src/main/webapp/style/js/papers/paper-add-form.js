...
function paperCommit(type) {
    ...
    for (let i = 1; i < nextDivIndex; i++) {
        ...
        if (jsonFormObject["qType" + i] === '0') {
            if (!fieldIsWrong(jsonFormObject["qa" + i + "Score"]) && !isNum(jsonFormObject["qa" + i + "Score"])) {
                layer.msg("第" + i + "题分数必须是数字");
                return false;
            }
            allInfo["paperQuestionList[" + index + "].questionScore"] = jsonFormObject["qa" + i + "Score"];
            allInfo["baseQuestionsList[" + index + "].options"] = getChooseOptionsStr(jsonFormObject, i);
    ...
    $.post("/Papers/savePaper.do", allInfo,
        function (data) {
            if (data.status === 201) {
...
function formItemValid(jsonFormObject) {
    console.log(isNum(jsonFormObject.examTimeNum));
